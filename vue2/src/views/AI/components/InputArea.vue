...
const emits = defineEmits(["scroll", "query"]);
...
function query() {
  // console.log(content.value, "query");
  emits("query", content.value);
}
</script>

<template>
  <el-input placeholder="请输入内容" clearable autofocus @keydown.native.enter="query" v-model="content">
    <template #append>
      <el-button icon="el-icon-search" @click.trim="query"></el-button>
    </template>
  </el-input>
</template>
